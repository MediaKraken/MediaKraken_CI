...
def hit_hb(s, dumpf, host, quiet):
    while True:
        ...
        if typ is None:
            print
            'No heartbeat response received from ' + host + ', server likely not vulnerable'
            return False
        ...
        if typ == 24:
            ...
            if len(pay) > 3:
                ...
                print
                'Server ' + host + ' processed malformed heartbeat, but did not return any extra data.'
            return True
        ...
        if typ == 21:
            ...
            hexdump(pay, dumpf, quiet)
            print
            'Server ' + host + ' returned error, likely not vulnerable'
            return False
...
def parseresp(s, quiet):
    while True:
        ...
        if typ == None:
            print
            'Server closed connection without sending Server Hello Done.'
            sys.stdout.flush()
            return 0
        ...
        if typ == 22:
            ...
            while index < len(pay):
                if not quiet: print
                'Message Type is 0x%02X' % ord(pay[index])
                sys.stdout.flush()
...
def check(host, port, dumpf, quiet, starttls):
    ...
    if starttls:
        ...
        try:
            ...
            print
            'Timeout issues, going ahead anyway, but it is probably broken ...'
        tls(s, quiet)
    else:
        s = connect(host, port, quiet)
        tls(s, quiet)
...
def main():
    ...
    print
    'Scanning ' + args[0] + ' on port ' + str(opts.port)
